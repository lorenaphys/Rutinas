...
for i=1:length(xf)
    for j=1:length(xf)
yf(i,j)=((1/(Nx/8))*(sqrt((xf(i)-Nx/2)^2+(xf(j)-Ny/2)^2))).^2;
zf(i,j)=1*(2/(sqrt(3*Nx/8)*pi^0.25)) * exp(-yf(i,j)/2) .* (1-yf(i,j));
        ...
        for i=1:Nx
            for j=1:Ny
                for k=1:Nz
                    ...
                    bet(i,j,k)=2.5*exp(-(25-k)/30)*zf(i,j);
                    
%                     bet(i,j,k)=1+3.5*exp(-((i-Nx/2)^2+(j-Ny/2)^2+(k-R1+8)^2)/80);
%                     if R1>15 & k<(R1-15)
...
%         fix0(:,:)=fi(Nx/2,:,:);
        ...
        %       fi=fi-dt*(F+Fs);
        I=60*sum(sum(sum((fi>=0))))/Nx/Ny/Nz*(F);
